...
% TODO: Update the code to be consistent with the changes in popCard and isEmpty

\java{CardCollection} also provides \java{isEmpty}, returns \java{true} if there are no cards left:

\begin{code}
public boolean isEmpty() {
    return cards.isEmpty()  //ArrayList method isEmpty
}
\end{code}
...
In order to control the ways card collections are modified, we don't provide a wrapper for the \java{ArrayList} method \java{set}.
...
%slr: 1-19-20 added the folloswing paragraph:
Importantly, this establishes the \textbf{is-a} relationship between \java{Deck} and \java{CardCollection}: \java{Deck} \textbf{is-a} \java{CardCollection}.  \java{Deck} adds more, so far a \java{Deck} constructor.
...
Like \java{Deck}, the \java{Hand} class extends \java{CardCollection}.  Like \java{Deck}, \java{Hand} \textbf{is-a} \java{CardCollection}.
...
In summary, a \java{Deck} is a \java{CardCollection}, but it provides a different constructor.
And a \java{Hand} is a \java{CardCollection}, but it provides a different constructor and an additional method, \java{display}.
...
It's because \java{Hand} is a subclass of \java{CardCollection}, so a \java{Hand} object is also considered to be a \java{CardCollection} object.  This is the is-a relationship:  \java{Hand} is-a \java{CardCollection}.
...
Every cat is-a mammal, and every mammal is-a animal.
...
A \java{Player} has two \java{private} attributes: a name and a hand.  This is the \textbf{has-a} relationship and is \textbf{not} an example of class inheritance:  \java{Player} \textbf{has-a} \java{Hand}, but it is not true that \java{Player} \textbf{is-a} \java{Hand}.
