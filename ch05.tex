...
%slr:  8-15-19 We'll also take a look at the \java{Math} class, which provides methods for common mathematical operations.
...
%slr:  8-15-19
Here is an example of the use of relational operators on /java{int} variables:
% height = 130 + 11 * num_lines
\begin{trinket} [270] {RelationalOperators.java}
...
Run the program and see if you agree with the results.  Change the values and/or types of x and y and see if you can predict the outcome.  Try, as above, comparing incompatible types to see what happens.  These relational operators only really work properly with the 8 java primitive types. For now, avoid using them with \java{String}s.
%slr:  end 8-15-19
...
\begin{enumerate}
\item Run the program.  Does it run as expected?  Change the value of x to an even number to test \textbf{both branches} of the \java{if - else} statement.
\item Mess around with the program in the trinket.  Try some conditional statements involving both \java{x} and \java{y}.  
\item Try some of the above versions of the \java{if} and \java{else} statements, including the broken ones. Make certain you understand the ramifications of misplaced semicolons!!
\item Search for, as suggested above, Apple's 'goto fail' bug.  What, exaclty, was the error?  What coding practice regarding if statements could have precluded this error?
\end{enumerate}
%slr: end 8-15-19
...
% height = 130 + 11 * num_lines
\begin{trinket} [270] {ChainedIfElse.java}
...
Change the value of x above to make sure all \textbf{three} branches work and that only \textbf{one} of the blocks executes.
%slr: end 8-15-19
...
%slr:  8-15-19
% height = 130 + 11 * num_lines
\begin{trinket} [300] {NestedIfElse.java}
...
\textbf{Section Exercises}
\begin{enumerate}
\item Run the program.  Does it run as expected?  Change the value of x above to make sure all \textbf{three} branches work and that only \textbf{one} of the blocks executes.
\item Run the following trinket and repeat what you did in the first exercise above.  Does it work properly?  What do you think of it?
\end{enumerate}
% height = 130 + 11 * num_lines
\begin{trinket} [200] {NestedIfElse.java}
public class NestedIfElse {
...
%slr:  8-15-19
Lets try an example.  What do you predict the output of the following program will be?
% height = 130 + 11 * num_lines
\begin{trinket} [270] {ChainedIfWithLogicalOperators.java}
public class ChainedIfWithLogicalOperators {
...
\textbf{Section Exercises}
\begin{enumerate}
\item Run the program.  Does it run as expected?  Change the values of x and y above to make sure all \textbf{three} branches work and that only \textbf{one} of the blocks executes.
\item Rewrite the program with nested, rather than chained, if-else statements.
\end{enumerate}
%slr: end 8-15-19
...
% height = 130 + 11 * num_lines
\begin{trinket} [330] {BooleanMethod.java}
public class BooleanMethod {
    ...
    public static void main(String[] args) {
       int x = 4; 
       if(isSingleDigit(x)) {  //not if( isSingleDigit(x) == true ) {
           System.out.println("x is a single digit integer.");
       } else {
           System.out.println("x is a multi-digit integer.");
       }
    }
}
\end{trinket}

Test the method to make sure it works correctly.
%slr: end 8-15-19
...
%slr:  8-15-19
Change the \java{isSingleDigit} in the trinket above to reflect this change.  Test it to make sure it still works correctly.
%slr: end 8-15-19
...
% height = 130 + 11 * num_lines
\begin{trinket} [250] {ComputeLog.java}
...
%slr:  8-15-19
Make the change in the trinket above and see how it works not.
%slr: end 8-15-19
...
%slr:  8-15-19
Try it and see!  You get a runtime exception:
%slr: end 8-15-19
...
\textbf{Section Exercises}
\begin{enumerate}
\item Add the above input error checking to the trinket (named ComputeLog, above).  Test it thoroughtly, both with negative values and with non-double values.
\end{enumerate}
%slr: end 8-15-19
...
%slr:  8-15-19
It should be more or less the same as the version you wrote in the last Section Exercise:

\index{Logarithm.java}

\begin{trinket} [ 470] {Logarithm.java}
import java.util.Scanner;
