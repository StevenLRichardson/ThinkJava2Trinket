...
\begin{enumerate}
\item Modify the the above trinket \java{Card}.  Add definitions for the class variables \java{RANKS} and \java{SUITS} at the very top of the \java{Card} class as shown above.  Also modify the \java{toString} method as shown.  Test your modified trinket to make sure it still works the same way as before.
\end{enumerate}
%slr: end 1-11-20
...
\textbf{Section Exercises:}
\begin{enumerate}
\item Now make further modifications to those you made in the previous section to the \java{Card} class.  First add the \java{compareTo} \textbf{instance} method as shown immediately above.  Then modify the \java{main} method so as to declare two \java{Card} objects, compare them with your new method and report the results. (Call this instance method on an instance and pass another as a parameter as the method definition clearly indicates - for example \java{a.compateTo(b);}.
\end{enumerate}
%slr: end 1-11-20
...
\textbf{Section Exercises:}
\begin{enumerate}
\item Now make yet further modifications to those you made in the previous section exercises to the \java{Card} class.  Modify the declarations for the two instance variables to be \java{final}.  Then add getters.  Make sure the trinket will still compile and run.  Finally, demonstrate that your \java{Card} objects are in fact immutable by adding setters that try to modify the values.  What happens when you try to compile the program?
\end{enumerate}
%slr: end 1-11-20
%slr:  insert trinket implementaton of above.  In Section Exercise ask that they demonstrate immutability by providing, say, a setter
...
%slr: trinket which access a null object and tries to prin it.
...
%slr: 1-11-20: 
Here is a trinket which builds an array of \java{Card} objects and prints them out:

% height = 130 + 11 * num_lines
\begin{trinket} [640] {Card.java}
public class Card {
    //class attributes
    private int rank;
    private int suit;
...
%slr:  Section Exercises:  1. fix trinket above by initializing the array of Card objects and show it worked by printing one out.  2.  Implement and test the printDeck method above.  3.  Try the Arrays.toString method as shown above.
...
\textbf{Section Exercises:}
\begin{enumerate}
\item Modify the version of the \java{Card} class as implemented in Section~\ref{cardarray}.  Add the static \java{search} method and test it.  (You may want to omit the call to \java{printDeck}).
\end{enumerate}
%slr: end 1-11-20
...
% slr 1-11-20:  I agree, but can't think of one right now.
...
This algorithm only depends on the \java{compareTo} method of the object, so we can use this code with any object type that provides \java{compareTo}.
