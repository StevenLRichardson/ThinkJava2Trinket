...
\begin{trinket} [230] {ImmutableString1.java}
...
%slr: 12-12-19
% height = 130 + 11 * num_lines
\begin{trinket} [230] {ImmutableString2.java}
...
%System.out.println(name);
%\end{code}
In the above trinket, the \java{String} object that \java{name} referenced is \textit{replaced} at line 5 with a referemce to a \textbf{new} \java{String} - the \java{String} returned by \java{toUpperCase()}.
...
%slr: 12-12-19
% height = 130 + 11 * num_lines
\begin{trinket} [230] {Replace.java}
public class Replace {

    public static void main(String[] args) {
       String text = "Computer Science is fun!";
       text = text.replace("Computer Science", "CS"); //store result of replace into text
...
If you don't assign the return value, the new \java{String} produced by the call is effectively abandoned - created on the heap but never assigned to any \java{String} object it is orphaned and will be deleted eventually by the Java runtime system.
...
\begin{enumerate}
\item Modify the trinket \java{ImmutableString1} so that it stores the result of invoking \java{name.toUpperCase()} into a \java{String} object hamed \java{upper} and prints the result.
\end{enumerate}
%slr: end 12-12-19
...
%slr: 12-12-19
% height = 130 + 11 * num_lines
\begin{trinket} [230] {Integer1.java}
...
Predict the output, and then run the trinket to see if you were correct.
%slr: end 12-12-19
...
%slr: 12-12-19
% height = 130 + 11 * num_lines
\begin{trinket} [300] {Integer2.java}
...
% height = 130 + 11 * num_lines
\begin{trinket} [300] {ParseAndToString.java}
public class ParseAndToString {
    ...
    public static void main(String[] args) {
       ...
       String str = "12345";
       int num = Integer.parseInt(str);
       System.out.printf("parse result is %d\n", num); //print int variable
       //convert int to String:
       num = 12345;
       str = Integer.toString(num);
       System.out.printf("parse result is %s\n", str); //print String object
    }
}
\end{trinket}
...
% height = 130 + 11 * num_lines
\begin{trinket} [210] {NumberFormatExceptionError.java}
...
a run-time exception will occur. Run the trinket to see what happens.
...
%slr: 12-15-19
Here is a \java{main} method:
\begin{code}
public static void main(String[] args) {
   //statements makin up the main method
}
\end{code}

So \java{main} is a \java{public static void} method which means it returns nothing (and is public and static). It has a single input paramenter named \java{args} of type \java{String []}.  So \java{args} is an array of \java{String}.  Where does this array of \java{String} come from?  The command line.

As an example let's write a program to find the maximum value in a sequence of numbers.
...
These values were passed as part of the command line above which invokes the \java{main} method of class \java{Max}.

It's not clear from the output, but the elements of \java{args} are strings.
So \java{args} is the array \java{{"10", "-3", "55", "0", "14"}}.

Back to our task of finding the maximum number: to do so we have to convert the \java{String} arguments to integers.
...
%slr: 12-16-19
Here is a complete example. The trinket envirionment does not allow us to specify command-line parameters which populate the \java{args String} array with values, so we do so manually.  (We are abusing our input parameter a bit here!)

% height = 130 + 11 * num_lines
\begin{trinket} [400] {Max.java}
//determine the maximum value of the command-line arguments
public class Max {
    
    public static void main(String[] args) {
       //populate the args array with values:
       //simulate receiving command-line arguments
       String[] temp = {"1", "2", "3", "4"};
       args = temp;
       if (args.length == 0) {
          System.err.println("Usage: java Max <numbers>");
       ...
       //find and report largest command-line argument value
       int max = Integer.MIN_VALUE;
       for (String arg : args) {
          int value = Integer.parseInt(arg);
          if (value > max) {
             max = value; //'high water mark' algorithm
          }
       }
       System.out.println("The max is " + max);
    }
...
Run the trinket to make sure it is working properly.

\textbf{Section Exercises:}
\begin{enumerate}
\item Modify the trinket \java{Max} to test that it works correctly with no command-line arguments supplied.  You will have to remove (or comment out) the statements that simulate receiving command-line arguments.
\item True or false?: Even even if no command-line arguments are received, args is never \java{null}.
\item What happens if your simulated user (who supplies the command-line arguments) provides one or more values that are not parsable as \java{int} values?  Modify the trinket to test and see.
\item Draw a memory diagram (refer to figures 9.1, 9.2, 9.3 and 9.4) of the \java{String} arrays \java{temp} and \java{args} after the statement \java{args = temp}.
\end{enumerate}
...
If \java{str} is \java{null}, the \java{||} operator will \textit{short circuit} and evaluate to \java{true} immediately.
...
% height = 130 + 11 * num_lines
\begin{trinket} [400] {ArgumentValidation.java}
public class ArgumentValidation {
    ...
    public static void main(String[] args) {
       String name = null;
       System.out.println(isCapitalized(name));
    }
}
\end{trinket}

Predict what the output will be, then run it to see if you were right.

\textbf{Section Exercises:}
\begin{enumerate}
\item Modify the trinket \java{ArgumentValidation} to test that it works correctly with an empty \java{String}.
\item Modify the trinket \java{ArgumentValidation} to test that it works correctly with a non-empty \java{String} with a non-capitalized first character.
\item Modify the trinket \java{ArgumentValidation} to test that it works correctly with a non-empty \java{String} with a capitalized first character.
\end{enumerate}
%slr: end 12-16-19
...
Here is an example:
% height = 130 + 11 * num_lines
\begin{trinket} [400] {UsingBigInteger.java}
import java.math.BigInteger;
public class UsingBigInteger {
    public static void main(String[] args) {
    
       BigInteger a = BigInteger.valueOf(17);
       BigInteger b = new BigInteger("1700000000");
...
Run the trinket to see what is output.
%slr: end 12-15-19

Like \java{String}s, \java{BigInteger} objects are immutable.
...
%slr:  12-16-19
Here is a complete program demonstrating this generalization:
% height = 130 + 11 * num_lines
\begin{trinket} [400] {MultTable.java}
...
Predict the output of the trinket and then run it to see if you were correct.
...
\textbf{Section Exercises:}
\begin{enumerate}
\item When you generalize a method appropriately, you often find that it has capabilities you did not plan.
For example, you might notice that the multiplication table is symmetric.
Since $ab = ba$, all the entries in the table appear twice.
You could save ink by printing half of the table:
\begin{stdout}
   1
   2   4
   3   6   9
   ...
   7  14  21  28  35  42  49
\end{stdout}
Modify the trinket so as to produce this result. (You should only have to change \textit{one line}!)
\end{enumerate}
%slr: end 12-16-19
