...
The program below declares two variables with type \java{String} and two with type \java{int}.
...
\end{code}
%slr: added fourth assignment statement above
...
%begin slr: discuss the two steps of an assignment statement
\index{assignment!two steps of}
...
Here are the two steps for the statement \java{hour = hour + 1;} above:
\begin{itemize}
\item First the right-hand-side \java{hour + 1} is evaluated.  The value of \java{hour} is 
fetched from its storage location.  Then \java{1} is added to the fetched value.
\item The result of step 1 is placed into the variable (storage location) named \java{hour}.
\end{itemize}
%end slr
...
%slr:
In mathematics, the equation $x = x + 1$ as a statement of equality false.  In java \java{x = x + 1;} is an assignment statement, {\bf not} a statement of equality.
...
%slr:  in-line exercises
\textbf{Section Exercises:}
\begin{enumerate}
\item Run the program.  What is the output?
\item Try changing the type of hour (and/or minute) to \java{String}. Can you get it to work?
...
\begin{enumerate}
\item Run the program.  What is the output?
\item Try changing the values of x and y.  See if you can successfully predict the output if you make, for example, x = 23 and y = 6
\item What does the division operator yield for the expression \java{34 / 5}? 
\item What does the remainder operator yield for this expression: \java{34 % 5}? 
\item What does the division operator yield for the expression \java{-34 / 5}? 
\item What does the remainder operator yield for this expression: \java{-34 % 5}?
\end{enumerate}
%end slr:
...
% slr: some changes and added trinket  8-10-19
...
we see that the right-hand side \java{1/3} is evaluated first with and yields the value 0!  This is integer arithmetic.
...
In the first line, \java{1 + 2} is \java{3} (because both \java{1} and \java{2} are integers), and \java{3 + "Hello"} is \java{"3Hello"} (because one operand is of type String so the other is converted to the same type).
