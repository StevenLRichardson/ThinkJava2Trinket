...
Run it and see what it dispays.  You should see
...
In {\tt Hello.java}, the outermost braces contain the {\em class definition}, and the inner braces contain the {\em method definition}.
...
In this context, the high-level program is called the {\bf source code}. The translated program is called the {\bf object code} or the {\bf executable} and is stored by the compiler for execution.
Once a program is compiled, you can execute it repeatedly without further translation.
As a result, compiled programs often run faster than interpreted programs.
%%begin slr:

Compiled programs are portable because on each different machine the compiler translates the source code into the particular machine's object code.  But if you want to move a program from machine A to machine B, you need to recompile it on machine B.  The object code for machine A will not (in general) run on machine B.

\index{virtual machine}
\index{byte code}
Java addresses the need to recompile on different machines by compiling code for a \textbf{virtual machine}.  This imaginary machine's object code is called \textbf{byte code}.
...
Where is this interpreter?  Each installation of java on a particular machine includes the \textbf{java virtual machine} or JVM.  Compiled java programs are stored as instructions for the virtual machine, and when the JVM 'executes' these instructions they are interpreted into the local machine's language and executed.
...
%But it is also portable, so it is possible to compile a Java program on one machine, transfer the byte code to another machine, and run the byte code on the other machine.  This is because the byte code is locally translated to machine-native language. This ability is an advantage of Java over some other high-level languages.
%%%slr end
...
%The Java interpreter is a program named {\tt java}, which is short for ``Java Virtual Machine'' (JVM).  It translates byte-code into local executable code.
%%slr end
...
%%begin slr (some changes in this paragraph)
The programmer writes source code in the file {\tt Hello.java} and uses the java compiler {\tt javac} to compile it.
If there are no errors, the compiler saves the byte code in the file {\tt Hello.class}.
To run the program, the programmer uses the java vritual machine {\tt java} to interpret the byte code.
...
%%begin slr:
Run this example and see what it produces.
%%slr end

The first statement does not add a newline, so the output appears on a single line as {\tt Goodbye, cruel world}.
...
\begin{enumerate}
\item Predict what will happen if you omit the space in the string given the {\\tt print}  statement.  Make the change and run the program to see if you were right.
\item Change where you insert the space between {\tt Goodbye,} and {\tt cruel world}.
\item Predict what will happen if you replace the print method with another println.  Make the change and run the program to see if you were right.
\end{enumerate}
%%slr end
...
Run the program.  What happens?  Do you get an error message?  What is generating it?  Can you interpret it?  (You may want to widen the pane the message is printed in to see it without line wrapping.)
...
We could make it all one line:

% height = 130 + 15 * num_lines
\begin{trinket}[145]{Goodbye.java}
public class Goodbye { public static void main(String[] args){ System.out.print("Goodbye, ");System.out.println("cruel world");}}
\end{trinket}

Again, it still works, but now is \emph{really} hard to read.
%%slr end
...
%%begin slr:
How could we produce the following output?
...
Try running it.  Do you see the problem?  The character \java{"} has a special meaning.  It does not mean literally a double quote.  The java compiler interprets it as a \emph{delimiter} - a special character whose meaning is 'this is the beginning of a string' or 'this is the end of a string'.  Each pair of these characters denotes (or delimits) a string.

How can we tell java that we do not want it to interpret the character in this way?  We can {\bf escape} the default meaning of the character \java{"} by preceding it with a backslash  character:

% height = 130 + 15 * num_lines
\begin{trinket}[220]{Hello4.java}
public class Hello4 {
...
Run this to verify that it produces:
...
The string is delimited with \java{"} characters and contains the {\bf escape sequence} (\verb'\"') which java interprets as 'they mean a literal double quote character, \textbf{not} the beginning or end of a string.'
...
If you add a space there, there will be a space at the beginning of the second line. Try it!
